...
void GraphicsEngine::Render(const std::vector<glm::mat4> &boidData, bool drawSimulation, float simulationTime, float maxAltitude, float minAltitude, float earthRadius)
{
    glClearColor(0.05f, 0.05f, 0.1f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    // Camera floor is based on the physical Earth, not the boids' minimum flight altitude
    ...
    if (drawSimulation)
    {
        ...
        // Earth scales based purely on earthRadius
        ...
        // --- 4. Draw Boundaries (Inner and Outer) ---
        glEnable(GL_BLEND);
        ...
        // Draw Outer Boundary (Max Altitude)
        // Add a small visual offset so boids don't clip through the visual ceiling
        ...
        // Draw Inner Boundary (Min Altitude)
